    ...
    function (mocks) {
      ...
      describe('signWithDefaultCert', function () {
        it('should call exec with correct args', async function () {
          (mocks as any).fs.expects('exists').once().withExactArgs(apiDemosPath).returns(true);
          (mocks as any).helpers
          ...
          (mocks as any).apkSigningHelpers.expects('getApksignerForOs').returns(apksignerDummyPath);
          (mocks as any).adb
            .expects('executeApksigner')
            .once()
            .withExactArgs([
              'sign',
              '--key',
        ...
        it('should fail if apksigner fails', async function () {
          ...
          (mocks as any).adb
            ...
            .throws();
          (mocks as any).helpers.expects('getJavaForOs').once().returns(javaDummyPath);
      ...
      describe('signWithCustomCert', function () {
        it('should call exec with correct args', async function () {
          ...
          (mocks as any).fs.expects('exists').once().withExactArgs(keystorePath).returns(true);
          (mocks as any).fs.expects('exists').once().withExactArgs(apiDemosPath).returns(true);
          (mocks as any).apkSigningHelpers.expects('getApksignerForOs').returns(apksignerDummyPath);
          (mocks as any).adb
            .expects('executeApksigner')
        ...
        it('should fallback to jarsigner if apksigner fails', async function () {
          ...
          (mocks as any).teen_process
            ...
            .returns({});
          (mocks as any).helpers.expects('getJavaHome').returns(javaHome);
          // Mock unsignApk's dependencies: tempDir and zip operations
          (mocks as any).tempDir.expects('openDir').returns('/tmp/dummy');
      ...
      describe.skip('zipAlignApk', function () {
        it('should call exec with correct args', async function () {
          ...
          (mocks as any).tempDir
            .expects('path')
            .once()
            .withExactArgs({prefix: 'appium', suffix: '.tmp'})
            .returns(alignedApk);
          (mocks as any).adb.expects('initZipAlign').once().withExactArgs().returns('');
          (mocks as any).appiumSupport
            .expects('mkdirp')
            .once()
      ...
      describe('checkApkCert', function () {
        beforeEach(function () {
          (mocks as any).fs.expects('hash').returns(Math.random().toString(36));
        });
        ...
        it('should check default signature when not using keystore', async function () {
          ...
          (mocks as any).adb
            .expects('executeApksigner')
            .once()
            .withExactArgs(['verify', '--print-certs', apiDemosPath]).returns(`
        ...
        it('should fail if apksigner is not found', async function () {
          ...
          (mocks as any).fs.expects('exists').once().withExactArgs(apiDemosPath).returns(true);
          (mocks as any).apkSigningHelpers.expects('getApksignerForOs').throws();
          (mocks as any).helpers.expects('getJavaForOs').returns(javaDummyPath);
        ...
        it('should call getKeystoreHash when using keystore', async function () {
          ...
          (mocks as any).fs
            ...
            .once() // For apksigner.jar existence check in getBinaryFromSdkRoot
            .returns(true);
          (mocks as any).adb.expects('getKeystoreHash').once().returns({
