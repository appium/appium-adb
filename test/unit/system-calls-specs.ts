    ...
    describe('getConnectedDevices', function () {
      ...
      it('should get all connected devices with verbose output', async function () {
        (mocks as any).teen_process
          .expects('exec')
          .once()
          .withExactArgs(adb.executable.path, ['-P', '5037', 'devices', '-l'])
          .returns({
            stdout:
    ...
    describe('getDevicesWithRetry', function () {
      it('should fail when there are no connected devices', async function () {
        ...
        (mocks as any).teen_process
          .expects('exec')
          .atLeast(2)
          .withExactArgs(adb.executable.path, ['-P', '5037', 'kill-server']);
      ...
      it('should get all connected devices second time', async function () {
        (mocks as any).teen_process.expects('exec').onCall(0).returns({stdout: 'Foobar'});
        (mocks as any).teen_process
          .expects('exec')
          .withExactArgs(adb.executable.path, ['-P', '5037', 'devices'])
          .returns({stdout: 'List of devices attached \n emulator-5554	device'});
      ...
      it('should fail when exec throws an error', async function () {
        (mocks as any).teen_process.expects('exec').atLeast(2).throws('Error foobar');
        await expect(adb.getDevicesWithRetry(1000)).to.eventually.be.rejectedWith(
          /Could not find a connected Android device/,
        );
      });
...
describe(
  'System calls 2',
  withMocks({adb, B, teen_process}, function (mocks) {
    afterEach(function () {
      (mocks as any).verify();
    });
    ...
    it('ls should return list', async function () {
      (mocks as any).adb.expects('shell').once().withExactArgs(['ls', 'foo']).returns('bar');
    ...
    describe('reboot', function () {
      it('should call stop and start using shell', async function () {
        (mocks as any).adb.expects('shell').once().withExactArgs(['stop']);
        (mocks as any).adb
          .expects('setDeviceProperty')
          .once()
          .withExactArgs('sys.boot_completed', 0);
        (mocks as any).adb.expects('shell').once().withExactArgs(['start']);
        (mocks as any).adb
          .expects('getDeviceProperty')
          .once()
          .withExactArgs('sys.boot_completed')
          .returns('1');
        (mocks as any).B.expects('delay').once().withExactArgs(2000);
      ...
      it('should restart adbd as root if necessary', async function () {
        ...
        (mocks as any).adb
          ...
          .throws(
            new Error(
              `Error executing adbExec. Original error: 'Command 'adb shell stop' exited with code 1'; Stderr: 'stop: must be root'; Code: '1'`,
            ),
          )
          .onSecondCall()
          .returns();
      ...
      it('should throw original error if cause of error is something other than no root access', async function () {
        ...
        (mocks as any).adb.expects('shell').once().throws(new Error(originalError));
    ...
    describe('getRunningAVD', function () {
      it('should get connected avd', async function () {
        ...
        (mocks as any).adb
          ...
          .withExactArgs()
          .returns([emulator]);
        (mocks as any).adb.expects('setEmulatorPort').once().withExactArgs(port);
        (mocks as any).adb.expects('execEmuConsoleCommand').once().returns(avdName);
        (mocks as any).adb.expects('setDeviceId').once().withExactArgs(udid);
      ...
      it('should return null when expected avd is not connected', async function () {
        ...
        (mocks as any).adb
          ...
          .returns([emulator]);
        (mocks as any).adb.expects('setEmulatorPort').once().withExactArgs(port);
        (mocks as any).adb.expects('execEmuConsoleCommand').once().returns('OTHER_AVD');
        expect(await adb.getRunningAVD(avdName)).to.be.null;
      });
      it('should return null when no avd is connected', async function () {
        (mocks as any).adb.expects('getConnectedEmulators').once().withExactArgs().returns([]);
        expect(await adb.getRunningAVD(avdName)).to.be.null;
      });
    });
    ...
    describe('root', function () {
      it('should restart adb if root throws err and stderr contains "closed" in message', async function () {
        (mocks as any).adb.expects('isRoot').once().returns(false);
        (mocks as any).adb.expects('adbExec').once().withExactArgs(['root']).throws({
          stdout: '',
