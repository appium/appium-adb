...
/**
 * Calculates hash sum for the particular file
 *
 * @param {string} filePath The full path to a file.
 * @param {?string} algorithm [sha256] Any of the hash algorithm names
...
async function fileHash (filePath, algorithm = 'sha256') {
  return await new B((resolve, reject) => {
    ...
    const readStream = _fs.ReadStream(filePath);
    ...
    readStream.on('end', () => resolve(shasum.digest('hex')));
  });
}
