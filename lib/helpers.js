 ...
 */
const getAndroidPlatformAndPath = _.memoize(async function getAndroidPlatformAndPath () {
  ...
  for (const propsPath of propsPaths) {
    ...
    if (!match) {
      log.warn(`Cannot read the SDK version from '${propsPath}'. Skipping '${platformPath}'`);
      continue;
    }
