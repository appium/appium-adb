...
const getModuleRoot = _.memoize(function getModuleRoot () {
  const pathParts = path.resolve(__dirname).split(path.sep);
  for (let pathItemIdx = pathParts.length; pathItemIdx > 0; --pathItemIdx) {
    const currentDir = path.join(path.sep, ...(pathParts.slice(0, pathItemIdx)));
    const manifestPath = path.join(currentDir, 'package.json');
    try {
      if (!_fs.existsSync(manifestPath)) {
        continue;
      }
      if (JSON.parse(_fs.readFileSync(manifestPath, 'utf8')).name === 'appium-adb') {
        return currentDir;
      }
    } catch (ign) {}
  }
