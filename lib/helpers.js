...
const MAIN_INTENT = 'android.intent.action.MAIN';
...
function parseManifest (manifest) {
  ...
  for (const activity of [
    ...
    for (const {actions, categories} of activity.intentFilters) {
      ...
      const isMainAction = actions.some(({name}) => name === MAIN_INTENT);
 ...
 * activity name or null if it cannot be parsed
 */
function parseLaunchableActivityName (dumpsys) {
  const mainActivityNameRe = new RegExp(`^\\s*${_.escapeRegExp(MAIN_INTENT)}:$`);
  const launcherCategoryNameRe = new RegExp(
    `^\\s*Category: "${_.escapeRegExp(LAUNCHER_CATEGORY)}"$`);
  const blocks = [];
  ...
  for (const item of blocks) {
    if (!item.some((l) => launcherCategoryNameRe.test(l))) {
      continue;
    }

    const activityNameStr = _.trim(item[0]);
    if (!activityNameStr) {
      continue;
    }
...
export {
  ...
  parseLaunchableActivityName, matchComponentName,
};
