...
const getBuildToolsDirs = _.memoize(async function getBuildToolsDirs (sdkRoot) {
  const buildToolsDirs = await fs.glob(path.resolve(sdkRoot, 'build-tools', '*') + path.sep,
                                       {absolute: true});
  if (_.isEmpty(buildToolsDirs)) {
    log.info(`Found zero 'build-tools' folders under '${sdkRoot}'`);
    return [];
  }

  const pairs = await B.map(buildToolsDirs, async (dir) => [(await fs.stat(dir)).mtimeMs, dir]);
  const result = pairs
    .sort((a, b) => a[0] < b[0])
    .map((pair) => pair[1]);
  log.info(`Found the following 'build-tools' folders under '${sdkRoot}' (the newest comes first):`);
...
export { getAndroidPlatformAndPath, unzipFile, assertZipArchive,
         ...
         rootDir, androidPlatforms, getSdkToolsVersion, getApksignerForOs, getBuildToolsDirs };
