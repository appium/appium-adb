...
export async function readPackageManifest(apkPath) {
  ...
  log.debug(`Reading package manifest: '${util.quote([
  ...
  try {
    ({stdout} = await exec(
      (/** @type {import('@appium/types').StringRecord} */ (this.binaries)).aapt2, args
    ));
  } catch (e) {
  ...
  for (const line of stdout.split('\n')) {
    if (line.startsWith('package:')) {
      ...
      const value = extractValue(line, /sdkVersion:'([^']+)'/, toInt);
      if (value) {
