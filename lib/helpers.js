...
/**
 * Verifies whether any array item starts with the given string
 *
 * @param {string} s The expected prefix string
 * @param {Array<string>} arr The array of strings to be scanned
 * @return {boolean} True if any of array items starts with the given string prefix
 */
function startsWithAny (s, arr) {
  return arr.some((x) => _.startsWith(x, s));
}
...
function parseAapt2Strings (rawOutput, configMarker) {
  const extractContent = function (startIdx, allLines) {
    let idx = startIdx;
  ...
  while (lineIndex < allLines.length) {
    ...
    if (startsWithAny(trimmedLine, ['type', 'Package'])) {
      currentResourceId = null;
      isInPluralGroup = false;
    ...
    if (currentResourceId) {
      if (isInPluralGroup) {
        ...
        if (isInCurrentConfig) {
          const [content, idx] = extractContent(lineIndex, allLines);
          lineIndex = idx;
