...
/*
 * Checks SurfaceOrientation in "dumpsys input" output to determine the currently screen rotation
 * Possible values: 0 - PORTRAIT, 1 - LANDSCAPE
 * Default is 0
 */
function deviceOrientation (dumpsys) {
  let re = new RegExp(`SurfaceOrientation\\:\\s+[${orientations.LANDSCAPE}|${orientations.PORTRAIT}]`, 'gi');
  let m = re.exec(dumpsys);
  // if information is missing we assume screen is on portrait mode
  return (m && m.length > 0 && new RegExp(`${orientations.LANDSCAPE}$`).test(m[0])) ? 
...
export { getDirectories, getAndroidPlatformAndPath, unzipFile, assertZipArchive,
         ...
         rootDir, androidPlatforms, deviceOrientation, orientations };
