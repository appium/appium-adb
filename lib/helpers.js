 ...
 * @property {string} permission - The name of the corresponding permission
 * @property {?boolean} granted - Whether the permission is granted or not
 */

/**
 * Retrieves permissions list encoded in `dumpsys package`
 * command output. It is expected that the first line of
 * this output is the title of the corresponding permissions group
 * and it contains the starting indent.
 *
 * @param {string} dumpsysOutput - The actual command output
 * @returns {Array<PermissionInfo>} The list of matched items
 * or an empty list if no matches were found
 */
const extractPermissionsFromGroup = function (dumpsysOutput) {
  const lines = dumpsysOutput.split('\n');
  if (lines.length < 2) {
    return [];
  }

  const titleIndent = lines[0].search(/\S|$/);
...
export { getAndroidPlatformAndPath, unzipFile, assertZipArchive,
         ...
         getApkanalyzerForOs, getOpenSslForOs, extractPermissionsFromGroup };
