...
// This is fix for https://github.com/appium/appium/issues/5396 getting the devices propery when runnning with mulitiple appium instance
ADB.prototype.shellForgettingDeviceProperty = function (cmd, cb) {
  if (cmd.indexOf('"') === -1) {
    cmd = '"' + cmd + '"';
  }
  var execCmd = '-s '+this.udid+' shell ' + cmd;
  this.exec(execCmd, cb);
};
...
ADB.prototype.reboot = function (cb) {
  ...
  this.shellForgettingDeviceProperty(adbCmd, function (err) {
    if (err) return cb(err);
    ...
    async.until(
      ...
      function fn(cb) {
        ...
        this.shellForgettingDeviceProperty("getprop sys.boot_completed", function (err, stdout) {
          if (err) return cb(err);
