...
var spawn = require('child_process').spawn
  ...
  , logger = require('./logger')
  , Asdk = require('./devices/asdk.js')
  , Genymotion = require('./devices/genymotion.js')
...
ADB.prototype.getConnectedEmulators = function (cb) {
  ...
  this.getConnectedDevices(function (err, devices) {
    ...
    _.each(devices, function (device) {
      ...
      if (port) {
        device.port = port;
        device.deviceModel = this.getEmulatorType(device.udid);
...
ADB.prototype.getEmulatorType = function (udid) {
  var dm = null;
  for (var i = 0; i < this.deviceModels.length; i++) {
    if (this.deviceModels[i].isThisMe(udid)) {
      dm = this.deviceModels[i];
      break;
    }
  }
  return dm;
