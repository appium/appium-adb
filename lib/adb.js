...
ADB.logger = logger;
ADB.jars = {};
_(['move_manifest.jar', 'sign.jar', 'strings_from_apk.jar', 'unsign.jar',
...
ADB.prototype.processExists = function (processName, cb) {
  ...
  this.shell("ps", function (err, out) {
    ...
    logger.info("process: " + processName + " exists:" + exists);
    cb(null, exists);
  });
};
...
ADB.prototype.getFocusedPackageAndActivity = function (cb) {
  ...
  var cmd = "dumpsys window windows"
    ...
    , searchRe = new RegExp(
      /mFocusedApp.+ActivityRecord\{.*\s([^\s\/\}]+)\/([^\s\/\}]+)(\s[^\s\/\}]+)*\}/);
...
ADB.prototype.waitForActivityOrNot = function (pkg, activity, not,
  ...
  logger.info("Waiting for pkg \"" + pkg + "\" and activity \"" + activity +
...
ADB.prototype.setWifi = function (on, cb) {
  var cmd = 'svc wifi ' + (on === 0 ? 'disable' : 'enable')
  this.shell(cmd, cb);
};
...
ADB.prototype.setData = function (on, cb) {
  var cmd = 'settings put global mobile_data ' + on;
  this.shell(cmd, cb);
};
