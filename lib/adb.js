...
ADB.prototype.exec = function (cmd, opts, cb) {
  ...
  async.retry(retryNum, function (_cb) {
    exec(cmd, opts, function (err, stdout, stderr) {
      if (err) {
        var retryCheck = new RegExp("protocol fault \\(no status\\)","i").test(stderr);
        if (retryCheck) {
          logger.info("got protocol fault error, retrying: " + cmd);
          return _cb(new Error(stderr)); // get back into retry loop
        }
