...
Asdk.prototype.isThisMe = function (udid) {
  return udid.indexOf("emulator-") !== -1;
};

Asdk.prototype.sendTelnetCommand = function (command, cb) {
  logger.info("Sending telnet command to device: " + command);
  this.adb.getEmulatorPort(function (err, port) {
...
Asdk.prototype.getAvdName = function (emulator, cb) {
...
Asdk.prototype.getDefinedAvds = function (cb) {
  this.adb.checkSdkBinaryPresent("android", function (err, binaryLoc) {
    ...
    exec(binaryLoc + " list avd -c", function (err, stdout) {
      ...
      var avds = [];
      if (stdout) {
        logger.info("ADK getDefinedAvds:\n" + stdout.trim());
...
Asdk.prototype.launchAVD = function (avdName, avdArgs, language, locale, avdLaunchTimeout,
    avdReadyTimeout, cb, retry) {
  if (typeof retry === "undefined") {
    retry = 0;
  }
  logger.info("Launching ASDK:" + avdName + ", launchTimeout " +
              avdLaunchTimeout + "ms and readyTimeout " + avdReadyTimeout +
              "ms");
  this.adb.checkSdkBinaryPresent("emulator", function (err, emulatorBinaryPath) {
    if (err) return cb(err);
    ...
    this.adb.getRunningAVDWithRetry(avdName.replace('@', ''), avdLaunchTimeout,
        function (err) {
      if (err) {
        if (retry < 1) {
