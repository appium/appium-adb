...
apkUtilsMethods.getFocusedPackageAndActivity = async function () {
  ...
  const nullFocusedAppRe = new RegExp(/mFocusedApp=null/);
  // https://regex101.com/r/xZ8vF7/1
  const focusedAppRe = new RegExp('mFocusedApp.+Record\\{.*\\s([^\\s\\/\\}]+)' +
                                        '\\/([^\\s\\/\\}\\,]+)\\,?(\\s[^\\s\\/\\}]+)*\\}');
  const nullCurrentFocusRe = new RegExp(/mCurrentFocus=null/);
  const currentFocusAppRe = new RegExp('mCurrentFocus.+\\{\\w+\\s+([^\\s\\/]+)\\/([^\\s]+)');
  const getMatch = (pattern, output) => {
  ...
  try {
    ...
    for (const pattern of [focusedAppRe, currentFocusAppRe]) {
      const match = getMatch(pattern, stdout);
      if (match) {
        return {
