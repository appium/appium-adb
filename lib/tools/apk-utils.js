 ...
 *
 * @param {string} locale - The name of device country connected with `-`. e.g. en-US.
 */
...
apkUtilsMethods.setDeviceLanguageCountry = async function (language, country) {
  ...
  if (!hasLanguage && !hasCountry) {
    log.debug(`setDeviceLanguageCountry requires language or country.`);
    log.debug(`language: '${language}' and country: '${country}'`);
    return;
  }
  ...
  if (apiLevel < 23) {
    ...
    if (apiLevel === 23) {
      ...
      if (!hasCountry || !hasLanguage) {
        log.debug(`setDeviceLanguageCountry requires both language and country to be set for API 24+`);
        log.debug(`Got language: '${language}' and country: '${country}'`);
        return;
      }
