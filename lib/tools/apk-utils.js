...
apkUtilsMethods.getFocusedPackageAndActivity = async function () {
  ...
  const nullFocusedAppRe = new RegExp(/^mFocusedApp=null/, 'm');
  // https://regex101.com/r/xZ8vF7/1
  const focusedAppRe = new RegExp('^mFocusedApp.+Record\\{.*\\s([^\\s\\/\\}]+)' +
                                  '\\/([^\\s\\/\\}\\,]+)\\,?(\\s[^\\s\\/\\}]+)*\\}', 'm');
  const nullCurrentFocusRe = new RegExp(/^mCurrentFocus=null/, 'm');
  const currentFocusAppRe = new RegExp('^mCurrentFocus.+\\{\\w+\\s+([^\\s\\/]+)\\/([^\\s]+)', 'm');
