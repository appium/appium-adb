...
apkUtilsMethods.ensureCurrentLocale = async function ensureCurrentLocale (language, country, script) {
  ...
  return /** @type {boolean} */ (await retryInterval(5, 1000, async () => {
    ...
    if (hasLanguage && !hasCountry) {
      return new RegExp(languagePattern, 'i').test(actualLocale);
    }
    if (!hasLanguage && hasCountry) {
      return new RegExp(countryPattern, 'i').test(actualLocale);
    }
    return [languagePattern, countryPattern].every((p) => new RegExp(p, 'i').test(actualLocale));
  }));
};
