...
apkUtilsMethods.waitForActivityOrNot = async function (pkg, activity, not, waitMs = 20000) {
  if (!pkg || !activity) {
  ...
  for (let oneActivity of allActivities) {
    if (!oneActivity.startsWith('.')) {
      ...
      // otherwise add the package name
  ...
  /* jshint ignore:start */
  log.errorAndThrow(`${possibleActivityNames.map((name) => `'${name}'`).join(' or ')} never ${not ? 'stopped' : 'started'}`);
  /* jshint ignore:end */
};
