...
apkUtilsMethods.install = async function install (appPath, options = {}) {
  ...
  if (this.remoteAppsCacheLimit > 0) {
    ...
    const cacheApk = async () => await this.cacheApk(appPath, {
      timeout: options.timeout,
    });
    try {
      const cachedPath = await cacheApk();
      performAppInstall = async () => {
        ...
        const output = await this.shell(pmInstallCmdByRemotePath(cachedPath), installOpts);
        ...
        if (/\bINSTALL_FAILED_INSUFFICIENT_STORAGE\b/.test(output)) {
          ...
          await clearCache();
          const newCachedPath = await cacheApk();
          return await this.shell(pmInstallCmdByRemotePath(newCachedPath), installOpts);
        }
        return output;
      };
    } catch (e) {
