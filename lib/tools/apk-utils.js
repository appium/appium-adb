 ...
 * Compatibility for appium-android-driver 1.32.4-
 *
...
apkUtilsMethods.setDeviceLanguageCountry = async function (language, country) {
  ...
  if (apiLevel < 23) {
    ...
    let localeCountry;
    if (!haveCountry) {
      localeLang = language.toLowerCase();
      localeCountry = language.toUpperCase();
    } else if (!haveLanguage) {
      localeLang = country.toLowerCase();
      localeCountry = country.toUpperCase();
    } else {
      localeLang = language.toLowerCase();
      localeCountry = country.toUpperCase();
    }

    log.debug(`Current locale: '${curLocale}'; requested locale: '${localeLang}-${localeCountry}'`);
    if (`${localeLang}-${localeCountry}` !== curLocale) {
      await this.setDeviceSysLocaleViaSettingApp(localeLang, localeCountry);
    }
  }
