...
apkUtilsMethods.isAppInstalled = async function (pkg) {
  ...
  try {
    ...
    // It is mandatory to verify if `pkg` is the valid class name first
    // otherwise dumpsys always returns the default package information
    if (this.isValidClass(pkg)) {
      const stdout = await this.shell(['dumpsys', 'package', pkg]);
      isInstalled = !_.includes(stdout, 'Unable to find package:');
    }
