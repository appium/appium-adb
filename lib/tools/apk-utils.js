...
apkUtilsMethods.setDeviceLocale = async function (locale) {
  let validateLocale = new RegExp(/[a-zA-Z0-9]+-[a-zA-Z0-9]+/);
...
apkUtilsMethods.setDeviceLanguageCountry = async function (language, country) {
  ...
  if (wasSettingChanged) {
    log.info("To apply locale setting via setting persist.sys.locale command, reboot the target device is required.");
    await this.reboot();
  }
};
