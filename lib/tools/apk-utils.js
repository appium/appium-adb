 ...
 * This or activity is required in order to be able to launch an app. If not provided
 * then the API will try to resolve its name automatically for the given package.
 ...
 * @property {?number} waitDuration [20000] - The number of milliseconds to wait until the
...
apkUtilsMethods.startApp = async function startApp (startAppOptions = {}) {
  startAppOptions = _.cloneDeep(startAppOptions);
  if (!startAppOptions.pkg || !startAppOptions.action) {
    throw new Error('pkg and intent action are required to start an application');
  }

  if (!startAppOptions.activity) {
    log.info('Activity name is not provided. Trying to resolve it from the package');
    startAppOptions.activity = await this.resolveLaunchableActivity(startAppOptions.pkg);
  }
  if (startAppOptions.activity) {
