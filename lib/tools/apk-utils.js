import { buildStartCmd, APKS_EXTENSION } from '../helpers.js';
...
apkUtilsMethods.install = async function (appPath, options = {}) {
  ...
  if (options.grantPermissions) {
    const apiLevel = await this.getApiLevel();
    if (apiLevel < 23) {
      log.debug(`Skipping granting permissions for '${appPath}', since ` +
  ...
  const executeInstall = async (args) => {
    const output = await this.adbExec(['install', ...args, appPath], {timeout});
  ...
  try {
    return await executeInstall(additionalArgs);
  } catch (err) {
 ...
 */
apkUtilsMethods.getPackageName = async function (appPath) {
  return (await this.getApkInfo(appPath)).name;
};
