 ...
 *
 * @param {string} sensor - Sensor type: light, pressure, humidity, etc.
 * @param {float} value  - Number to set as the sensor value.
 */
emuMethods.sensorSet = async function sensorSet (sensor, value) {
  if (_.values(emuMethods.SENSORS).indexOf(sensor) === -1) {
    throw new Error(`Unsupported sensor sent '${sensor}'. Supported values: ${_.values(emuMethods.SENSORS)}]`);
  }
  if (!_.isNumber(value)) {
    throw new Error(`Missing sensor value argument. You need to provide a value number to set to the sensor.`);
  }
