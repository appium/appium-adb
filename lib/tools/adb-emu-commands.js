...
emuMethods.sendSMS = async function (phoneNumber, message, udid) {
  if (_.isUndefined(phoneNumber) &&
    !/^[\+]?[(]?[0-9]{3}[)]?[-\s\.]?[0-9]{3}[-\s\.]?[0-9]{4,}$/im.test(phoneNumber)) {
    log.errorAndThrow('sendSMS phoneNumber is undefined');
  }
  if (_.isUndefined(message) && _.isEmpty(message)) {
    log.errorAndThrow('sendSMS message is undefined');
  }
  await this.checkEmulatorConnected(udid);
  this.resetTelnetAuthToken();
  this.adbExec(['emu', 'sms', 'send', phoneNumber, message]);
};
