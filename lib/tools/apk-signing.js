 ...
 *                      This method assumes each argument is quoted correctly.
...
apkSigningMethods.signWithDefaultCert = async function (apk) {
  ...
  try {
    ...
    try {
      await exec(java, ['-jar', signPath, quotedApk, '--override']);
    } catch (e) {
...
apkSigningMethods.signWithCustomCert = async function (apk) {
  ...
  try {
    ...
    try {
      ...
      await exec(getJavaForOs(), ['-jar', path.resolve(this.helperJarPath, 'unsign.jar'), quotedApk]);
      ...
      await exec(jarsigner, ['-sigalg', 'MD5withRSA', '-digestalg', 'SHA1',
        ...
        '-keypass', this.keyPassword, quotedApk, this.keyAlias]);
    } catch (e) {
...
apkSigningMethods.checkApkCert = async function (apk, pkg) {
  ...
  try {
    ...
    verificationFunc = async () => {
      const output = await this.executeApksigner(['verify', '--print-certs', quotedApk]);
      if (!_.includes(output, DEFAULT_CERT_DIGEST)) {
        throw new Error(`${quotedApk} is signed with non-default certificate`);
      }
    };
  } catch (e) {
    ...
    const java = getJavaForOs();
    verificationFunc = async () => await exec(java, ['-jar', path.resolve(this.helperJarPath, 'verify.jar'), quotedApk]);
  }
  try {
    await verificationFunc();
    log.debug(`${quotedApk} is already signed.`);
    return true;
  } catch (e) {
    log.debug(`${quotedApk} is not signed with debug cert.`);
    return false;
  }
};
