...
apkSigningMethods.zipAlignApk = async function zipAlignApk (apk) {
  ...
  try {
    await exec((/** @type {import('@appium/types').StringRecord} */ (this.binaries)).zipalign, ['-c', '4', apk]);
  ...
  try {
    await exec(
      (/** @type {import('@appium/types').StringRecord} */ (this.binaries)).zipalign,
      ['-f', '4', apk, alignedApk]
    );
...
apkSigningMethods.checkApkCert = async function checkApkCert (appPath, pkg, opts = {}) {
  ...
  if (SIGNED_APPS_CACHE.has(appHash)) {
    ...
    // @ts-ignore These props should exist
