...
const PASSWORD_CANNOT_BE_NULL_OR_EMPTY_ERROR = `Credential can't be null or empty`;
const PASSWORD_DID_NOT_MATCH_ERROR = `didn't match`;
 ...
 * command line tool. This tool has been added to Android toolset since  API 28 Oreo
 *
...
lockManagementMethods.isLockManagementSupported = async function isLockManagementSupported () {
  if (!_.isBoolean(this._isLockManagementSupported)) {
    try {
      await this.shell(['locksettings', 'help']);
      log.debug(`Extended lock settings management is supported`);
      this._isLockManagementSupported = true;
    } catch (ign) {
      log.debug(`Extended lock settings management is not supported`);
      this._isLockManagementSupported = false;
    }
  }
 ...
 */
lockManagementMethods.verifyLockCredentials = async function verifyLockCredentials (credential = null) {
  try {
    const output = await this.shell(buildCommand('verify', credential));
    return _.includes(output, 'verified successfully');
  } catch (e) {
    if (_.includes(e.stderr || e.stdout, PASSWORD_CANNOT_BE_NULL_OR_EMPTY_ERROR)) {
      return false;
    }
    throw e;
  }
 ...
 */
lockManagementMethods.clearLockCredentials = async function clearLockCredentials (credential = null) {
  try {
    ...
    throw new Error(`Cannot clear device lock settings. Original error: ${e.message}`);
  }
};
...
lockManagementMethods.isLockEnabled = async function isLockEnabled () {
  try {
    ...
    if ([PASSWORD_DID_NOT_MATCH_ERROR, PASSWORD_CANNOT_BE_NULL_OR_EMPTY_ERROR]
