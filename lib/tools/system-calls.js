...
systemCallMethods.installMitmCertificate = async function (cert) {
  const openSslBinaryName = await getOpenSslForOs();
  ...
  try {
    ...
    let {stdout} = await exec(openSslBinaryName, ['x509', '-noout', '-hash', '-in', tempCert.path]);
    const certHash = stdout.trim();
    log.debug(`Got certificate hash: ${certHash}`);
    log.debug('Preparing certificate content');
    ({stdout} = await exec(openSslBinaryName, ['x509', '-in', tempCert.path], {isBuffer: true}));
    let dstCertContent = stdout;
    ({stdout} = await exec(openSslBinaryName, ['x509',
      '-in', tempCert.path,
      '-text',
      '-fingerprint',
      '-noout'], {isBuffer: true}));
    ...
    try {
      ...
      await this.shell(['mount', '-o', 'rw,remount', '/system']);
      log.debug(`Uploading the generated certificate from '${dstCert.path}' to '${CERTS_ROOT}'`);
      await this.push(dstCert.path, CERTS_ROOT);
