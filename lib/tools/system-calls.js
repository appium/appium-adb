...
systemCallMethods.adbExec = async function (cmd, opts = {}) {
  ...
  try {
    ...
    if (!DEVICE_CONNECTING_ERROR_REGEXP.test(err)) {
      throw err;
    }

    log.info(`Error sending command, device is still connecting. Retrying`);
    return await retryInterval(5, 1000, execFunc);
  }
};
