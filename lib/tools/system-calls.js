...
systemCallMethods.installMitmCertificate = async function (cert) {
  const binaryName = system.isWindows() ? 'openssl.exe' : 'openssl';
  try {
  ...
  try {
    ...
    let {stdout} = await exec(binaryName, ['x509', '-noout', '-hash', '-in', tempCert.path]);
    const certHash = stdout.trim();
    log.debug(`Got certificate hash: ${certHash}`);
    log.debug('Preparing certificate content');
    ({stdout} = await exec(binaryName, ['x509', '-in', tempCert.path], {isBuffer: true}));
    let dstCertContent = stdout;
    ({stdout} = await exec(binaryName, ['x509',
      '-in', tempCert.path,
      '-text',
      '-fingerprint',
      '-noout'], {isBuffer: true}));
    ...
    try {
      ...
      log.debug(`Uploading the generated certificate from '${dstCert.path}' to '${dstPath}'`);
      await this.push(dstCert.path, dstPath);
