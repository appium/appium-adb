...
systemCallMethods.shell = async function shell (cmd, opts = {}) {
  ...
  if (privileged) {
    ...
    const wasAlreadyRooted = await this.isRoot();
    if (wasAlreadyRooted) {
...
systemCallMethods.launchAVD = async function launchAVD (avdName, opts = {}) {
  ...
  if (supportsDelayAdbFeature) {
    try {
      await this.adbExec(['wait-for-device'], {timeout: launchTimeout + readyTimeout});
    } catch (e) {
      try {
        await proc.stop();
      } catch (ign) {}
      throw new Error(`'${avdName}' Emulator has failed to boot within ${launchTimeout + readyTimeout}ms`);
    }
  } else {
    await retry(retryTimes, async () => await this.getRunningAVDWithRetry(avdName, launchTimeout));
  }
  // this wait is probably unneeded if hasDelayAdbFeature is true
  ...
  const retryIfOffline = async (cmdFunc) => {
    try {
      ...
      if (['closed', 'device offline', 'timeout expired']
        ...
        log.warn(`Attempt to ${cmd} caused device to go offline. Restarting adb`);
        await this.restartAdb();
        return await retryInterval(5, 400, cmdFunc);
      } else {
        throw err;
      }
    }
