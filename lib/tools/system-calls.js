...
systemCallMethods.fileSize = async function (remotePath) {
  try {
    let stdout = await this.shell(['ls', '-la', remotePath]);
    let lines = stdout.split('\n');
    ...
    let match = /[-rwxd]{10}\s*\S*\s*\S*\s*\S*\s*(\d*)\s*\d{4}-\d{2}-\d{2}\s\d{2}:\d{2}\s[\S\.]*/.exec(lines[0]);
    if (!match || _.isNaN(parseInt(match[1], 10))) {
      throw new Error(`Unable to parse size from list output: '${lines[0]}'`);
    }
