...
systemCallMethods.getConnectedDevices = async function () {
  ...
  try {
    ...
    if (startingIndex === -1) {
      log.errorAndThrow(`Unexpected output while trying to get devices. output was: ${stdout}`);
    } else {
...
systemCallMethods.getEmulatorPort = async function () {
  ...
  try {
    ...
    if (port) {
      ...
      log.errorAndThrow(`Emulator port not found`);
    }
  } catch (e) {
...
systemCallMethods.getRunningAVD = async function (avdName) {
  try {
    ...
    if (emulators.length < 1) {
      log.errorAndThrow("No emulators connected");
    }
  ...
  try {
    ...
    while ((Date.now() - start) < timeoutMs) {
      try {
        ...
        // Do nothing.
        log.info('Trying to get running AVD');
      }
      // cool down
      await sleep(200);
    }
...
systemCallMethods.launchAVD = async function (avdName, avdArgs, language, locale,
  ...
  let proc = child_process.spawn(emulatorBinaryPath, launchArgs);
  proc.on("error", function (err) {
    log.error("Unable to start Emulator: " + err.message);
    // actual error will get caught by getRunningAVDWithRetry
  });
  proc.stderr.on('data', function (data) {
    log.error("Unable to start Emulator: " + data);
  });
...
systemCallMethods.waitForDevice = async function (appDeviceReadyTimeout = 30) {
  ...
  await retry(3, async () => {
    ...
    try {
      await this.adbExec('wait-for-device', {timeout: timeoutSecs * 1000});
      await this.ping();
    } catch (e) {
      await this.restartAdb();
...
systemCallMethods.reboot = async function () {
  await this.shell(['stop;', 'sleep 2;', 'setprop sys.boot_completed 0;', 'start']);
  await retryInterval(90, 1000, async () => {
    let stdout = await this.shell(['getprop', 'sys.boot_completed']);
    if (stdout.trim() === '1') {
      return;
    } else {
