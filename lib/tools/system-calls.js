 ...
 */
systemCallMethods.getSdkBinaryPath = _.memoize(async function getSdkBinaryPath (binaryName) {
 ...
 */
systemCallMethods.getCommandForOS = _.memoize(function getCommandForOS () {
...
systemCallMethods.getBinaryFromSdkRoot = async function getBinaryFromSdkRoot (binaryName) {
  if (_.has(this.binaries, binaryName)) {
  ...
  if (this.sdkToolsVersion) {
    buildToolsDirs = buildToolsDirs
      .filter((x) => path.basename(x) === this.sdkToolsVersion);
    if (_.isEmpty(buildToolsDirs)) {
      log.info(`Found no build tools matching to '${this.sdkToolsVersion}' version`);
    } else {
      log.info(`Using build tools ${JSON.stringify(buildToolsDirs)} matching ` +
        `to '${this.sdkToolsVersion}' version`);
    }
  }
  ...
  if (_.isNull(binaryLoc)) {
    throw new Error(`Could not find ${fullBinaryName} in ${binaryLocs}. ` +
      `Do you have Android SDK Tools ${this.sdkToolsVersion ? `v ${this.sdkToolsVersion} ` : ''}` +
