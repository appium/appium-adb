...
manifestMethods.targetSdkVersionFromManifest = async function targetSdkVersionFromManifest (appPath) {
  ...
  for (const [index, [toolName, versionGetter]] of versionGetters.entries()) {
    try {
      return await versionGetter();
    } catch (e) {
      const msg = `Cannot extract targetSdkVersion of '${originalAppPath}' using ${toolName}. ` +
        `Original error: ${e.message}`;
      if (index >= versionGetters.length - 1) {
        throw new Error(msg);
      }
      log.info(msg);
    }
