...
manifestMethods.processFromManifest = async function (localApk) {
  try {
    ...
    log.error(`processFromManifest failed. Original error: ${e.message}`);
...
manifestMethods.packageAndLaunchActivityFromManifest = async function (localApk) {
  try {
    ...
    log.error(`packageAndLaunchActivityFromManifest failed. Original error: ${e.message}`);
...
manifestMethods.compileManifest = async function (manifest, manifestPackage, targetPackage) {
  ...
  try {
    ...
    log.error(`Error compiling manifest. Original error: ${err.message}`);
...
manifestMethods.insertManifest = async function (manifest, srcApk, dstApk) {
  ...
  try {
    ...
    log.error(`Error insterting manifest. Original error: ${e.message}`);
    throw new Error(`Error insterting manifest. Original error: ${e.message}`);
  }
};
...
manifestMethods.hasInternetPermissionFromManifest = async function (localApk) {
  try {
    ...
    log.error(`Error checking internet permission for manifest. Original error: ${e.message}`);
