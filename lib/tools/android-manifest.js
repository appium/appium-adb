 ...
 *                 application package.
 */
async function extractApkInfoWithApkTools (localApk) {
  await this.initAapt();
  ...
  if (apkActivity && apkActivity.length >= 2) {
    ...
    let getLaunchActivity = [
      '-jar', this.jars['appium_apk_tools.jar'],
      'printLaunchActivity', localApk,
      outputPath
    ];
    let {stdout, stderr} = await exec('java', getLaunchActivity);
    if (stderr) {
      throw new Error(`Cannot parse launchActivity from manifest: ${stderr}`);
    }
 ...
 */
async function extractApkInfoWithApkanalyzer (localApk) {
  const apkAnalyzer = await getApkanalyzerForOs(this);
  const args = ['-h', 'manifest', 'print', localApk];
  log.debug(`Starting '${apkAnalyzer}' with args ${JSON.stringify(args)}`);
  const manifestXml = (await exec(apkAnalyzer, args, {
    shell: true,
    cwd: path.dirname(apkAnalyzer)
  })).stdout;
...
manifestMethods.packageAndLaunchActivityFromManifest = async function (localApk) {
  const apkInfoGetters = [
    async () => await extractApkInfoWithApkanalyzer(localApk),
    async () => await extractApkInfoWithApkTools(localApk),
  ];
  ...
  for (const infoGetter of apkInfoGetters) {
    try {
      ...
      log.debug(`Package name: ${apkPackage}`);
      log.debug(`Main activity name: ${apkActivity}`);
      return {apkPackage, apkActivity};
    } catch (e) {
