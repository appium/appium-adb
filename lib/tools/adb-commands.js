...
/**
 * Fetches the fully qualified name of the main activity for the
 ...
 * @return {string} Fully qualified name of the main activity
 * @throws {Error} If there was an error while resolving the activity name
 */
methods.resolveMainActivity = async function resolveMainActivity (pkg) {
  ...
  for (const line of (stdout || '').split('\n')) {
    ...
    if (this.isValidClass(trimmedLine)) {
      return trimmedLine;
    }
  }
  throw new Error(`Unable to resolve the main activity of '${pkg}'. Original error: ${stderr || stdout}`);
};
