...
import { getIMEListFromOutput, isShowingLockscreen, isCurrentFocusOnKeyguard,
         getSurfaceOrientation, isScreenOnFully, extractPermissionsFromGroup } from '../helpers.js';
...
methods.grantAllPermissions = async function (pkg, apk) {
  ...
  if (apiLevel >= 23 && targetSdk >= 23) {
    ...
    let cmds = [];
    let cmdChunk = [];
    for (let permission of permissionsToGrant) {
...
methods.getGrantedPermissions = async function (pkg, cmdOutput = null) {
  ...
  for (const pattern of [
    ...
    new RegExp(/^(\s*runtime permissions:[\s\S]+)/, 'm'),
  ]) {
    ...
    grantedPermissions.push(
      ...(extractPermissionsFromGroup(match[1])
        .filter((item) => item.granted === true)
        .map((item) => item.permission)));
  }
  if (_.isEmpty(grantedPermissions)) {
    log.info(`No permissions were granted for '${pkg}'`);
  }
...
methods.getDeniedPermissions = async function (pkg, cmdOutput = null) {
  ...
  if (_.isEmpty(deniedPermissions)) {
    log.info(`No permissions were denied for '${pkg}'`);
  }
...
methods.getReqPermissions = async function (pkg, cmdOutput = null) {
  const stdout = cmdOutput || await this.shell(['dumpsys', 'package', pkg]);
  const match = new RegExp(/^(\s*requested permissions:[\s\S]+)/, 'm').exec(stdout);
  if (!match) {
    log.info(`It looks like '${pkg}' requests no permissions`);
    return [];
  }
  return extractPermissionsFromGroup(match[1])
    .map((item) => item.permission);
};
