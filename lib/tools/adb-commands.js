...
methods.push = async function (localPath, remotePath) {
  await this.adbExec(['push', localPath, remotePath]);
};
...
methods.processExists = async function (processName) {
  try {
    if (!this.isValidClass(processName)) {
      log.errorAndThrow(`Invalid process name: ${processName}`);
    }
...
methods.ping = async function () {
  try {
    ...
    log.errorAndThrow(`ADB ping failed. Orginial error: ${e.message}`);
  }
};
...
methods.killProcessesByName = async function (name) {
  try {
    ...
    if (pids.length < 1) {
      throw new Error(`No ${name} process found to kill`);
    }
    for (let pid of pids) {
...
methods.androidCoverage = async function (instrumentClass, waitPkg, waitActivity) {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      this.instrumentProc = child_process.spawn(this.adb.path, args);
      this.instrumentProc.on('error', function (e) {
        reject(new Error(`Failed to run instrumentation. Original error: ${e.message}`));
      });
      this.instrumentProc.stderr.on('data', function (data) {
        reject(new Error(`Failed to run instrumentation: ${new Buffer(data).toString('utf8')}`));
      });
