 ...
 *
 * @return {string} Device screen density as a number or _null_ if it
...
methods.getScreenDensity = async function () {
  ...
  if (density && density.length >= 2) {
    let densityNumber = Number(density[1].trim());
    return Number.isNaN(densityNumber) ? null : densityNumber;
  }
  return null;
};
