...
/**
 * Generate the standard bugreport output
 *
 * @param {?boolean} asString [true] - Whether to return the output
 *                                     as a raw string (the default behavior)
 *                                     or as a base64-encoded .zip archive (if set to `false`)
 * @param {?number} timeout [120000] - Command timeout in milliseconds
 * @returns {string} Either raw string output or base64-encoded .zip archive
 */
methods.bugreport = async function (asString = true, timeout = 120000) {
  if (asString) {
    return await this.adbExec(['bugreport'], {timeout});
  }

  const bugreportPath = await tempDir.path({prefix: 'bugreport', suffix: '.zip'});
  try {
    await this.adbExec(['bugreport', bugreportPath], {timeout});
    const data = await fs.readFile(bugreportPath);
    return Buffer.from(data).toString('base64');
  } finally {
    await fs.rimraf(bugreportPath);
  }
};
