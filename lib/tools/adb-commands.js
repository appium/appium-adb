...
methods.lock = async function lock () {
  ...
  log.debug('Pressing the KEYCODE_POWER button to lock the screen');
  ...
  try {
    ...
    log.debug(await this.shell(['dumpsys', 'window']));
    log.warn(
      `The device screen appears to be not locked after ${timeoutMs}ms timeout. Continuing anyway`
    );
  }
};
