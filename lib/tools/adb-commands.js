...
methods.getApiLevel = async function () {
  ...
  try {
    ...
    log.error(`Error getting device API level. Original error: ${e.message}`);
...
methods.stopAndClear = async function (pkg) {
  try {
    ...
    log.error(`Cannot stop and clear ${pkg}. Original error: ${e.message}`);
...
methods.availableIMEs = async function () {
  try {
    ...
    log.error(`Error getting available IME's. Original error: ${e.message}`);
...
methods.enabledIMEs = async function () {
  try {
    ...
    log.error(`Error getting enabled IME's. Original error: ${e.message}`);
...
methods.defaultIME = async function () {
  try {
    ...
    log.error(`Error getting default IME. Original error: ${e.message}`);
