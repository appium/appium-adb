...
methods.killProcessesByName = async function (name) {
  try {
    ...
    if (e.message.match(/No .* process found to kill/)) {
      log.info(`${e.message}, continuing...`);
      return;
    }
...
methods.setDeviceLanguage = async function (language) {
  return this.setDeviceProperty("persist.sys.language", language.toLowerCase());
};
...
methods.setDeviceCountry = async function (country) {
  return this.setDeviceProperty("persist.sys.country", country.toUpperCase());
};
