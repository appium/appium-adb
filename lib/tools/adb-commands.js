 ...
 * }
 * ```
 * @throws {Error} If there was an error while getting the notfications list
 */
methods.getNotifications = async function getNotifications () {
  log.debug('Retrieving notifications');
  let output;
  try {
    output = await this.shell([
      'am', 'broadcast',
      '-n', NOTIFICATIONS_RECEIVER,
  ...
  if (!/\bresult=-1\b/.test(output) || !/\bdata="{/.test(output)) {
    log.debug(output);
    throw new Error('Cannot retrieve notifications from the device. ' +
      'Check the device log for more details.');
  }
  const match = /\bdata=("{.+)/.test(output);
  if (!match) {
    throw new Error(
      `Cannot parse notifications from the command output: '${output}'`);
  }
  ...
  try {
    ...
    throw new Error(
      `Cannot parse notifications from the resulting data string: '${jsonStr}'`);
  }
};
