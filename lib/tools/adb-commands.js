...
async function executeInImeContext (adb, imeId, fn) {
  const defaultIME = await adb.defaultIME();
  await adb.setIME(imeId);
  try {
    return await fn();
  } finally {
    await adb.setIME(defaultIME);
  }
}
...
methods.performEditorAction = async function performEditorAction (action) {
  ...
  await executeInImeContext(this, APPIUM_IME,
...
methods.getClipboard = async function getClipboard () {
  ...
  try {
    output = (await this.getApiLevel() >= 29)
      ? (await executeInImeContext(this, APPIUM_IME, retrieveClipboard))
