...
import { sleep, waitForCondition, retryInterval } from 'asyncbox';
...
methods.setIME = async function setIME (imeId) {
  ...
  await retryInterval(10, 300, async () => {
    if (!(await this.enabledIMEs()).includes(imeId)) {
      throw new Error(`IME ${imeId} has not been set after the timeout occurred`);
    }
  });
};
