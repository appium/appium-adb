...
methods.getGeoLocation = async function () {
  ...
  try {
    output = await this.shell([
      'am', 'broadcast',
      '-a', `${SETTINGS_HELPER_ID}.location`,
      '-n', `${SETTINGS_HELPER_ID}/.receivers.LocationInfoReceiver`,
    ]);
  } catch (err) {
    throw new Error(`Cannot retrieve the current geo coordinates from the device. ` +
      `Make sure the Appium Settings application is installed and has location permissions. Also the location ` +
