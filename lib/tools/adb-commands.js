...
methods.setGeoLocation = async function (location, isEmulator = false) {
  if (isEmulator) {
    this.adbExec(['emu', 'geo', 'fix', ''+ location.longitude, '' + location.latitude]);
  } else {
    return await this.shell(['am', 'startservice', '-e', 'longitude', '' + location.longitude,
                             '-e', 'latitude', '' + location.latitude, `${SETTINGS_HELPER_ID}/.LocationService`]);
  }
};
