...
import { getIMEListFromOutput, isShowingLockscreen, isCurrentFocusOnKeyguard,
         isScreenOnFully, deviceOrientation } from '../helpers.js';
...
methods.isScreenLandscape = async function () {
  try {
    ...
    return deviceOrientation(stdout) === 1 ? true : false;  
  }catch (e) {
...
methods.beLandscape = async function () {
  await this.setOrientation(1);
};

methods.bePortrait = async function () {
  await this.setOrientation(0);
};
