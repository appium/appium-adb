...
methods.getApiLevel = async function getApiLevel () {
  if (!_.isInteger(this._apiLevel)) {
    try {
      const strOutput = await this.getDeviceProperty('ro.build.version.sdk', true);
...
methods.getPlatformVersion = async function getPlatformVersion () {
  ...
  try {
    return await this.getDeviceProperty('ro.build.version.release', true);
  } catch (e) {
 ...
 * @param {boolean} memoize [false] - Whether to memoize the resulting value
 * for the current ADB instance.
 *
...
methods.getDeviceProperty = async function getDeviceProperty (property, memoize = false) {
  if (memoize) {
    ...
    if (this._memoizedDeviceProperties.has(property)) {
      return this._memoizedDeviceProperties.get(property);
    }
  }
  const val = _.trim(await this.shell(['getprop', property]));
  log.debug(`Current device property '${property}': ${val}`);
  if (memoize) {
    this._memoizedDeviceProperties.set(property, val);
  }
  return val;
};
...
methods.getModel = async function getModel () {
  return await this.getDeviceProperty('ro.product.model', true);
};
...
methods.getManufacturer = async function getManufacturer () {
  return await this.getDeviceProperty('ro.product.manufacturer', true);
};
