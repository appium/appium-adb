 ...
 * on: true (to turn on) or false (to turn off)
 */
methods.setWifiState = async function (on, isSimulator = false) {
  if (isSimulator) {
 ...
 * on: true (to turn on) or false (to turn off)
 */
methods.setDataState = async function (on, isSimulator = false) {
  if (isSimulator) {
 ...
 */
methods.setWifiAndData = async function ({wifi, data}, isSimulator = false) {
  if (!_.isUndefined(wifi)) {
    this.setWifiState(wifi, isSimulator);
  }
  if (!_.isUndefined(data)) {
    this.setDataState(data, isSimulator);
  }
};
