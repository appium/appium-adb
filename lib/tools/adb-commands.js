...
import { getIMEListFromOutput, isShowingLockscreen, isCurrentFocusOnKeyguard,
         isScreenOnFully, deviceOrientation, orientations } from '../helpers.js';
...
methods.setAutoRotationState = async function(on) {
  on = on && on === 'on' ? 1 : 0;
  let args =  ["content", "insert", "--uri", "content://settings/system",
              "--bind", "name:s:accelerometer_rotation", "--bind", `value:i:${on}`];
  await this.shell(args);
};

methods.isScreenLandscape = async function () {
  try {
    let stdout = await this.shell(['dumpsys', 'input']);
    return deviceOrientation(stdout) === orientations.LANDSCAPE ? true : false;  
  }catch (e) {
    log.errorAndThrow(`Error getting device orientation. Original error: ${e.message}`);
  }
};

methods.isScreenPortrait = async function () {
  return !(await this.isScreenLandscape());
};

methods.setOrientation = async function (orientation) {
  orientation = orientation === orientations.LANDSCAPE ? orientations.LANDSCAPE : orientations.PORTRAIT;
  let args = ["content", "insert", "--uri", "content://settings/system",
              "--bind", "name:s:user_rotation", "--bind", `value:i:${orientation}`];
  await this.shell(args);
};
