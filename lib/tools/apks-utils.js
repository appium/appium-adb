...
import { getJavaForOs, unzipFile, fileHash, buildInstallArgs, APKS_INSTALL_TIMEOUT } from '../helpers.js';
...
async function extractFromApks (apks, dstPath) {
  ...
  // as caching keys
  const apksHash = await fileHash(apks);
  ...
  if (APKS_CACHE.has(apksHash)) {
    return path.resolve(APKS_CACHE.get(apksHash), ...dstPath);
  }
 ...
 * executing it
 */
apksUtilsMethods.execBundletool = async function (args, errorMsg) {
 ...
 */
apksUtilsMethods.getDeviceSpec = async function (specLocation) {
  const args = [
    'get-device-spec',
