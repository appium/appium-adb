...
import {
  ...
  APKS_INSTALL_TIMEOUT, DEFAULT_ADB_EXEC_TIMEOUT
} from '../helpers.js';
...
apksUtilsMethods.installApks = async function installApks (apks, options = {}) {
  ...
  const args = [
    ...
    '--adb', this.executable.path,
    '--apks', apks,
    '--timeout-millis', options.timeout,
    '--device-id', this.curDeviceId,
  ];
  const {
    grantPermissions,
  ...
  await this.execBundletool(args, `Cannot install '${path.basename(apks)}' to the device ${this.curDeviceId}`);
  if (grantPermissions) {
    // TODO: Simplify it after https://github.com/google/bundletool/issues/246 is implemented
    const {name} = await this.getApkInfo(apks);
    await this.grantAllPermissions(name);
  }
};
