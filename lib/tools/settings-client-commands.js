...
commands.refreshGeoLocationCache = async function refreshGeoLocationCache (timeoutMs = 20000) {
  ...
  if (timeoutMs > 0) {
    ...
    const timeoutErrorMsg = `The GPS has not been refreshed within ${timeoutMs}ms timeout. ` +
      `Please make sure the device under test has Appium Settings helper installed and running. ` +
    ...
    monitoringPromise = new B((resolve, reject) => {
      ...
      const linesHanlder = (lines) => {
        if (lines.some((line) => GPS_CACHE_REFRESHED_LOGS.some((x) => line.includes(x)))) {
          resolve();
        }
      };
      ['lines-stderr', 'lines-stdout'].map((evt) => logcatMonitor.on(evt, linesHanlder));
    });
  ...
  if (logcatMonitor && monitoringPromise) {
    ...
    log.info('The request to refresh the GSP cache has been sent. Skipping waiting for its result.');
  }
};
